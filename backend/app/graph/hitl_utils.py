...
def _is_awaiting_hitl(snapshot: object) -> bool:
    ...
    HITL is handled inside the action_executor node via interrupt().
    The graph pauses with 'action_executor' in next_nodes when awaiting human approval.

    Pinned to langgraph==1.2.4 semantics; re-verify on version bump.
    """
    next_nodes = getattr(snapshot, "next", ()) or ()
    return "action_executor" in next_nodes
